...
- **多模型支持**  
  兼容 OpenAI、DeepSeek、Gemma 4 以及任何 OpenAI 兼容接口，通过 `.env` 文件轻松切换。
...
| 音频对齐      | MoviePy + NumPy                         |
| 大模型集成    | OpenAI SDK（DeepSeek, Gemma 4, OpenAI） |
| 计算机视觉    | OpenCV                                  |
| 环境变量      | python-dotenv                           |
...
新建 `.env` 文件，选择你的大模型提供商，并填入对应的 API 密钥：

```ini
# 大模型提供商：openai / deepseek / gemma4
...
# Gemma 4 通过 Google AI Studio（中国用户需代理）
...
- 设置 `LLM_PROVIDER=deepseek` 可直接使用 DeepSeek。  

- 如果使用 Gemma 4，需要 VPN/代理，并在 `.env` 中设置 `HTTPS_PROXY=http://127.0.0.1:你的VPN端口`，或cmd中运行
